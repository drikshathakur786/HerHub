...
class SignUpViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        // Sign Up button — native filled style
        signUpButton?.layer.cornerRadius = 12
        signUpButton?.clipsToBounds = true
        signUpButton?.titleLabel?.font = UIFont.systemFont(ofSize: 17, weight: .semibold)
        signUpButton?.backgroundColor = UIColor(red: 0.92, green: 0.38, blue: 0.68, alpha: 1.0)
        
        // "Already have an account? Sign In" — attributed
        styleSignInLink()
    ...
    private func styleSignInLink() {
        guard let btn = signInButton else { return }
        ...
        let attributed = NSMutableAttributedString(string: "Already have an account? ", attributes: regularAttrs)
        attributed.append(NSAttributedString(string: "Sign In", attributes: boldAttrs))
    ...
    private func addLogo() {
        ...
        NSLayoutConstraint.activate([
            ...
            logoImageView.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 152)
        ])
    }
