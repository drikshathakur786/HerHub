...
/// Manages local JSON storage for user data
final class UserJsonManager {
    
    static let shared = UserJsonManager()
    ...
    func createUser(_ user: User) throws {
        ...
        if users.contains(where: { $0.id == user.id || $0.email == user.email }) {
            throw NSError(domain: "UserJsonManager", code: -1,
    ...
    private func initializeTestUserIfNeeded() {
        if storage.exists(fileName: fileName) {
            print("📁 [UserJson] User data already exists")
            return
        }
        
        print("🌱 [UserJson] Initializing test user...")
        ...
        print("✅ [UserJson] Test user initialized: test@herhub.com")
    }
}
